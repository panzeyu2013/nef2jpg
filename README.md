...
尼康 SDK 需要 25 个色彩配置文件（19 个 `.icm` 等；`CCMSService` 初始化必需，缺失时报 `OpenLibrary 0x8`）。发布包附安装脚本：
...
- **自包含**：macOS 发布包解压即用（lib/ 已打包全部动态库），无需目标机 Homebrew。
- **签名/公证**：CI 产物为 adhoc 签名。正式分发建议用 Developer ID 签名并公证（`codesign` + `notarytool`），否则用户需右键打开或 `xattr -d com.apple.quarantine`。
- **许可**：尼康 Image SDK 为授权产品，随包再分发前请核对 SDK 许可协议；exiv2 为 LGPL-2.1；libjpeg-turbo 为 IJG/BSD 类许可。源码发布不受影响。
- **CI**：macOS 构建需提供 SDK 压缩包 URL（workflow 输入 `sdk_tarball_url` 或仓库变量 `SDK_TARBALL_URL`）。
